...
**한 세션에서는 한 주제만 다룬다.** 같은 주제라면 정리 글·실습 글·그에 딸린 질문을
한 세션에서 이어가도 된다. 주제가 바뀌면 세션을 새로 연다.

| 한 세션에서 해도 되는 것 | 세션을 새로 열어야 하는 것 |
| --- | --- |
| "섹션2 ACID" 정리 글 + 실습 글 | 섹션2 끝내고 **섹션3** 시작 |
| 그 글을 쓰다 생긴 질문과 검증 | 관련 없는 다른 주제 질문 |
| 질문에서 나온 내용을 그 글에 반영 | 다른 섹션·다른 강의 글 |

**왜 글 단위가 아니라 주제 단위인가.** 실제로 굴려 보니 질문 → 검증 → 글 개선이
계속 왔다 갔다 했고, 그때 쌓인 맥락이 글을 좋게 만들었다.
`t_xmin > t_xmax`, XID 배정 시점, 격리 수준 혼합 같은 내용은 전부 질문에서 나왔다.
글마다 세션을 끊으면 이게 끊긴다.

반대로 **주제가 바뀌면 반드시 끊는다.** 앞 주제의 자료·용어·검증 결과가 남아 있는 채로
다른 글을 쓰면 사실관계가 섞인다. 회사 자료가 끼어 있었다면 그게 다른 글로 새어 나간다.

### 질문에는 파일을 만들지 않는다

**답만 하고 끝낸다.** 질문은 글이 아니다.

- 검증이 필요하면 컨테이너를 띄우고 실제로 돌려 본다. 추측으로 답하지 않는다
- 답이 길어져도 **글로 만들지 않는다.** 남길 가치가 있어 보이면 그렇게 말만 하고,
  글에 넣을지는 사용자가 정한다
- 지금 세션의 주제와 **같은** 글에 반영하는 것은 괜찮다.
  다른 주제의 글을 새로 만드는 것은 안 된다

질문에서 나온 내용을 글에 넣기로 했다면, **넣기 전에 반드시 실제로 돌려서 확인한다.**
대화에서 맞는 말 같았다고 그대로 옮기지 않는다 — 이 규칙 덕분에
"RR 이 write skew 를 막는다", "SERIALIZABLE 하나만 올려도 된다" 같은
틀린 설명이 글에 들어가기 전에 걸러졌다.
